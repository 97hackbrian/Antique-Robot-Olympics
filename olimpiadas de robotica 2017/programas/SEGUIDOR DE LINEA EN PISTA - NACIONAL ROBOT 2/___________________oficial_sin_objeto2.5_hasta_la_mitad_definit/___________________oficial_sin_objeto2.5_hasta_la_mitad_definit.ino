...
if((position==7000)&&(e==0))
{
 //digitalWrite(13,HIGH);
 analogWrite(pwma,140); // VELOCIDAD PARA EL MOTOR IZQUIERDO
 analogWrite(pwmb,120); //  VELOCIDAD PARA EL MOTOR DERECHO
 digitalWrite(ain1,HIGH);   ///RETROCEDE
 digitalWrite(ain2,LOW);
 digitalWrite(bin1,LOW);  ///FRENTE
 digitalWrite(bin2,HIGH);
 ...
 while(true)
 {
   qtrrc.read(sensorValues);
     position = qtrrc.readLine(sensorValues, QTR_EMITTERS_ON, 0);
     if((sensorValues[5]==1000)||(sensorValues[4]==1000)&&(sensorValues[7]<=17))
     {
  //     digitalWrite(13,LOW);
      break;
     }
...
while(true)
 {
   qtrrc.read(sensorValues);
     position = qtrrc.readLine(sensorValues, QTR_EMITTERS_ON, 0);
     if((sensorValues[0]<=17)&&(sensorValues[2]==1000)||(sensorValues[3]==1000)||(sensorValues[7]==1000))
     {
      break;
     }
 }
