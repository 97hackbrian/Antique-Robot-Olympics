...
if((sensorValues[0]==1000)&&(sensorValues[1]==1000)&&(sensorValues[2]==1000)&&(sensorValues[3]==1000)&&(sensorValues[4]==1000)&&(sensorValues[5]==1000)&&(sensorValues[6]==1000)&&(sensorValues[7]==1000))
{
 ...
 digitalWrite(bin1,HIGH);  ///FRENTE
 digitalWrite(bin2,LOW);
 analogWrite(pwma,17); // VELOCIDAD PARA EL MOTOR DERECHO
 analogWrite(pwmb,90); //  VELOCIDAD PARA EL MOTOR IZQUIERDO
 delay(970);

}

if((sharp1==4)||(sharp1==5)||(sharp1==6)||(sharp1==7)||(sharp1==8)||(sharp1==9)||(sharp1==10)||(sharp1==11)&&(sensorValues[4]==1000))
{
...
while(true)
{
 ...
 digitalWrite(bin1,HIGH);  ///FRENTE
 digitalWrite(bin2,LOW);
 analogWrite(pwma,77); // VELOCIDAD PARA EL MOTOR DERECHO
 analogWrite(pwmb,28); //  VELOCIDAD PARA EL MOTOR IZQUIERDO
...
vel=(P*0.065)+(D*0.04)+(I*0.00400);// para velocidad 120//////estaba en 0.0925

//vel=(P*0.0428)+(D*0.085)+(I*0); //para velocidad 80 kd=0.06

///CRUZERO =VELOCIDAD PUNTA , V

    if(vel >cruzero2) vel=cruzero2;
    if(vel<-cruzero2) vel=-cruzero2;

  analogWrite(pwma,cruzero2+vel); // VELOCIDAD PARA EL MOTOR DERECHO
...
{ 
  //gira a la derecha
 analogWrite(pwma,cruzero2+vel); // VELOCIDAD PARA EL MOTOR DERECHO
  analogWrite(pwmb,cruzero2-vel-I); //  VELOCIDAD PARA EL MOTOR IZQUIERDO
 digitalWrite(ain1,HIGH);   ///FRENTE
 digitalWrite(ain2,LOW);
 digitalWrite(bin1,LOW);  ///RETROCEDE
 digitalWrite(bin2,HIGH);
...
if((sensorValues[0]<=250)&&(sensorValues[4]==1000)&&(sensorValues[5]==1000)&&(sensorValues[6]==1000)&&(sensorValues[7]==1000))
{ 
  //gira a la derecha
 analogWrite(pwma,cruzero2+vel); // VELOCIDAD PARA EL MOTOR DERECHO
  analogWrite(pwmb,cruzero2-vel-I); //  VELOCIDAD PARA EL MOTOR IZQUIERDO
